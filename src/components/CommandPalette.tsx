...
import { Search, Map, MapPin, Globe, Eye, Command, ArrowRight, CornerDownLeft } from 'lucide-react';
...
interface CommandPaletteProps {
  ...
  campaign: Campaign;
  setSelectedElement: (elem: { type: 'region' | 'route' | 'poi' | 'comment' | null; id: string | null }) => void;
  setPan: (p: Point) => void;
...
export default function CommandPalette({
  ...
  campaign.regions.forEach(r => {
    items.push({
      ...
      type: 'region',
      action: () => {
        // Calculate center of polygon points for panning
        ...
        // Pan so center of region is at (0,0) offset
        setPan({ x: -(centerX - campaign.mapWidth / 2), y: -(centerY - campaign.mapHeight / 2) });
        setZoom(1.2);
  ...
  campaign.pois.forEach(p => {
    items.push({
      ...
      type: 'poi',
      action: () => {
        setPan({ x: -(p.x - campaign.mapWidth / 2), y: -(p.y - campaign.mapHeight / 2) });
        setZoom(1.5);
  ...
  campaign.routes.forEach(route => {
    items.push({
      ...
      action: () => {
        ...
        setPan({ x: -(pStart.x - campaign.mapWidth / 2), y: -(pStart.y - campaign.mapHeight / 2) });
        setZoom(1.2);
  ...
  items.push({
    ...
    type: 'command',
    action: () => setLayerVisibility(prev => ({ ...prev, regioes: !prev.regioes }))
  });
  items.push({
    id: 'cmd-layer-rotas',
    ...
    category: 'Ação Rápida',
    type: 'command',
    action: () => setLayerVisibility(prev => ({ ...prev, rotas: !prev.rotas }))
  });
  items.push({
    id: 'cmd-layer-pins',
    ...
    category: 'Ação Rápida',
    type: 'command',
    action: () => setLayerVisibility(prev => ({ ...prev, pins: !prev.pins }))
  });
  items.push({
    id: 'cmd-reset-view',
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center pt-24 bg-black/60 backdrop-blur-md">
          ...
          >
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        {item.type === 'region' ? <Globe size={15} className="opacity-70" /> : 
                         item.type === 'poi' ? <MapPin size={15} className="opacity-70" /> : 
                         item.type === 'route' ? <Map size={15} className="opacity-70" /> : 
                         <Eye size={15} className="opacity-70" />}
                        
                        <div className="flex flex-col">
                          <span className="text-xs">{item.name}</span>
                          <span className={`text-[9px] uppercase tracking-wider ${isSelected ? 'text-slate-800' : 'opacity-40'}`}>
                            {item.category}
                          </span>
                        </div>
                      </div>
            ...
            <div className="px-4 py-2 bg-black/10 border-t border-white/5 flex items-center justify-between text-[10px] opacity-40">
              <span className="flex items-center gap-1"><ArrowRight size={10} /> Use as setas ↑ ↓ para navegar e Enter para selecionar</span>
